...
public class ReportPostDetailController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		*/
		/*
		 * if(noList.contains(no)==false) { BoardDAO.getInstance().updateHit(no);
		 * noList.add(no); }
		 */
		PostVO vo = PostDAO.getInstance().getReportPostingByNo(no);		
		request.setAttribute("pvo", vo);
		/*
		 * request.setAttribute("url", "/board/post-detail.jsp"); return
		 * "/template/layout.jsp";
		 */
